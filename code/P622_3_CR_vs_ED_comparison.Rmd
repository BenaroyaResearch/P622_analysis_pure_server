...
library(gtExtras)
gtsave(overlap_summary_table, filename = file.path(outDir, "overlap_summary_table_per_sample_CR_vs_ED.html"))
...
# Prepare composition data for both methods
composition_data <- bind_rows(
  cell_metadata_CR %>%
    mutate(method = "CellRanger",
           cell_type_short = celltype_abbrev[cell_type]),
  cell_metadata_ED %>%
    mutate(method = "EmptyDrops",
           cell_type_short = celltype_abbrev[cell_type])
) %>%
  group_by(method, cell_type_short) %>%
  summarise(n = n(), .groups = "drop") %>%
  group_by(method) %>%
  mutate(pct = round(n / sum(n) * 100, 2)) %>%
  ungroup() %>%
  mutate(
    method         = factor(method, levels = c("CellRanger", "EmptyDrops")),
    cell_type_short = factor(cell_type_short, levels = names(celltype_colors))
  )
...
p1 <- ggplot(composition_data,
  ...
  labs(title    = "Cell Type Composition: CellRanger vs EmptyDrops",
       subtitle = paste0("CR: ", nrow(cell_metadata_CR), " cells | ",
                         "ED: ", nrow(cell_metadata_ED), " cells"),
       x        = NULL,
       y        = "% of Total Cells",
...
# Plot 2 — Grouped bar (absolute counts)
p2 <- ggplot(composition_data,
  ...
  scale_y_continuous(expand   = c(0, 0),
                     labels   = scales::comma) +
  labs(title    = "Cell Type Counts: CellRanger vs EmptyDrops",
       subtitle = paste0("CR: ", nrow(cell_metadata_CR), " cells | ",
                         "ED: ", nrow(cell_metadata_ED), " cells"),
       x        = "Cell Type",
       y        = "Number of Cells",
...
# Save both
ggsave(file.path(outDir, "composition_stacked_CR_vs_ED.png"),
       plot   = p1,
       width  = 8,
       height = 8,
       dpi    = 150)

ggsave(file.path(outDir, "composition_grouped_CR_vs_ED.png"),
       plot   = p2,
       width  = 12,
       height = 7,
       dpi    = 150)

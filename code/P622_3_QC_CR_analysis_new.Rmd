...
seurat_obj_harmony_integrated_CR <- readRDS(file = file.path(processedDataDir, "seurat_obj_harmony_integrated_CR_with_UMAP.rds"))
...
expr_matrix <- GetAssayData(seurat_obj_harmony_integrated_CR, assay = "RNA", slot = "counts")  
...
# save(AUC_scores, file = file.path(processedDataDir, "AUC_scores_for_P493_celltypes_harmony_integrated_CR_temp_260303.rda"))

# Add AUC scores to metadata
metadata <- seurat_obj_harmony_integrated_CR@meta.data %>%
  rownames_to_column(var = "cellbc") %>%
  inner_join(AUC_scores %>% rownames_to_column(var = "cellbc")) %>%
  column_to_rownames(var = "cellbc")

all(rownames(metadata) == rownames(seurat_obj_harmony_integrated_CR@meta.data))

seurat_obj_harmony_integrated_CR@meta.data <- metadata
...
df <- seurat_obj_harmony_integrated_CR@meta.data[, c("harmony_clusters", names(celltype_signatures))]

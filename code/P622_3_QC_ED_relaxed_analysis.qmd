---
title: "P622-3 10X fixed data on ED relaxed called cells: QC and analysis"
output: html_document
author: Naresh Doni Jayavelu
---

This is second iteration with very relaxed cutoff values for ED parameters for cell calling
...
P622_3_df_all_emptyDrops_out_filtered <- P622_3_df_all_emptyDrops_out %>%
  filter(FDR < 0.01) %>%
  filter(lower == 50 & retain == 200)

# remove this object to save memory
...
# merge samples
merged_seurat_obj_ED_relaxed_cells <- merge(x = seuratObjectsList[[1]], 
...
all(sum(sapply(seuratObjectsList, function(x) dim(x)[2])) == dim(merged_seurat_obj_ED_relaxed_cells)[2])

# save as an .rds object
# saveRDS(merged_seurat_obj_ED_relaxed_cells, file = file.path(processedDataDir, "merged_seurat_obj_ED_relaxed_cells.rds"))
# merged_seurat_obj_ED_relaxed_cells <- readRDS(file = file.path(processedDataDir, "merged_seurat_obj_ED_relaxed_cells.rds"))
...
# Extract metadata
metadata_beforeQC_ED <- merged_seurat_obj_ED_relaxed_cells@meta.data
...
seurat_obj <- merged_seurat_obj_ED_relaxed_cells
...
# save the data
# saveRDS(seurat_obj_filtered_unintegrated, file = file.path(processedDataDir, "seurat_obj_filtered_unintegrated_ED_relaxed.rds"))

seurat_obj_filtered_unintegrated <- readRDS(file = file.path(processedDataDir, "seurat_obj_filtered_unintegrated_ED_relaxed.rds"))
...
# saveRDS(seurat_obj_harmony_integrated_ED, file = file.path(processedDataDir, "seurat_obj_harmony_integrated_ED_relaxed.rds"))
...
# seurat_obj_harmony_integrated_ED <- FindClusters(seurat_obj_harmony_integrated_ED, resolution = 0.6, cluster.name = "harmony_clusters_res0.6")
...
# saveRDS(seurat_obj_harmony_integrated_ED, file = file.path(processedDataDir, "seurat_obj_harmony_integrated_ED_relaxed_with_UMAP.rds"))

table(Idents(seurat_obj_harmony_integrated_ED))

seurat_obj_harmony_integrated_ED <- readRDS(file = file.path(processedDataDir, "seurat_obj_harmony_integrated_ED_relaxed_with_UMAP.rds"))

# marker genes
cluster_markers_harmony_ED <- FindAllMarkers(seurat_obj_harmony_integrated_ED)
# save(cluster_markers_harmony_ED, file = file.path(processedDataDir, "cluster_markers_harmony_integrated_ED_relaxed_temp_260302.rda"))
...
### Read marker genes from P493

```{r Normalization Integration and Clustering }
 
filename_temp <- "/nfs/bioinformatics/workspace/ndonijayavelu/P493_3_analysis_server/output/EDout_results/data_for_others/P493_cluster_marker_genes.rds"
P493_cluster_markers <- readRDS(file = filename_temp)

# select top 200 genes for each cell type
P493_top200_markers_per_celltype <- P493_cluster_markers %>%
  filter(pct.1 > pct.2) %>%
  filter(pct.1 > 0.1) %>%
...
celltype_signatures <- P493_top200_markers_per_celltype %>%
  group_by(cell_type) %>%
...
# 1. Extract the expression matrix from your Seurat object
expr_matrix <- GetAssayData(seurat_obj_harmony_integrated_ED, assay = "RNA", slot = "counts")  
# AUCell prefers raw counts

# 2. Build gene rankings (per cell)
cell_rankings <- AUCell_buildRankings(expr_matrix, 
                                       nCores = 4, 
                                       plotStats = FALSE)

# 3. Calculate AUC for each signature
cell_AUC <- AUCell_calcAUC(celltype_signatures,    # your named list from before
                            cell_rankings,
                            aucMaxRank = ceiling(0.05 * nrow(cell_rankings)))
# aucMaxRank: top 5% of genes is standard, can adjust to 0.05-0.10

# 4. Extract AUC scores and add to Seurat metadata
AUC_scores <- as.data.frame(t(getAUC(cell_AUC)))

# save(AUC_scores, file = file.path(processedDataDir, "AUC_scores_for_P493_celltypes_harmony_integrated_ED_relaxed_temp_260303.rda"))

# Add AUC scores to metadata
metadata <- seurat_obj_harmony_integrated_ED@meta.data %>%
  rownames_to_column(var = "cellbc") %>%
  inner_join(AUC_scores %>% rownames_to_column(var = "cellbc")) %>%
  column_to_rownames(var = "cellbc")

all(rownames(metadata) == rownames(seurat_obj_harmony_integrated_ED@meta.data))

seurat_obj_harmony_integrated_ED@meta.data <- metadata

FeaturePlot(seurat_obj_harmony_integrated_ED, reduction = "umap.harmony", features = names(celltype_signatures), 
            min.cutoff = "q5", max.cutoff = "q95")

VlnPlot(seurat_obj_harmony_integrated_ED, features = names(celltype_signatures))
...
### Visualize signature scores for every cluster

```{r Normalization Integration and Clustering }

# Pull relevant columns from metadata
df <- seurat_obj_harmony_integrated_ED@meta.data[, c("harmony_clusters", names(celltype_signatures))]

# Plot one violin per signature
pp_plots <- lapply(names(celltype_signatures), function(sig) {
  
  ggplot(df, aes(x = harmony_clusters, y = .data[[sig]], fill = harmony_clusters)) +
    geom_violin(trim = TRUE, scale = "width") +
    stat_summary(fun = mean, geom = "crossbar",
                 width = 0.3, fatten = 1.5, color = "black") +
    theme_classic() +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1, size = 8),
      legend.position = "none"
    ) +
    labs(x = NULL, y = "AUC Score", title = sig)
})

wrap_plots(pp_plots, ncol = 4)
